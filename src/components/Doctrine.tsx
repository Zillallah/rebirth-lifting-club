"use client";

import { motion, useScroll, useTransform } from "framer-motion";
import { useRef } from "react";
import { RevealGroup, RevealItem, Reveal } from "./motion/Reveal";
import { XMark } from "./XMark";

export function Doctrine() {
  const ref = useRef<HTMLElement>(null);
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: ["start end", "end start"],
  });
  const xMarkY = useTransform(scrollYProgress, [0, 1], [80, -80]);
  const xMarkRotate = useTransform(scrollYProgress, [0, 1], [-3, 3]);

  return (
    <section
      ref={ref}
      id="doctrine"
      className="relative bg-[var(--color-canvas)] z-10 overflow-hidden min-h-[90vh] flex items-center py-32 md:py-48"
    >
      <motion.div
        className="absolute -right-12 md:-right-24 top-1/2 -translate-y-1/2 pointer-events-none"
        style={{ y: xMarkY, rotate: xMarkRotate }}
      >
        <XMark size={680} opacity={0.05} />
      </motion.div>

      <div className="relative w-full max-w-[1200px] mx-auto px-6 md:px-12">
        <Reveal>
          <div className="text-[var(--color-amber)] font-semibold text-[12px] tracking-[0.32em] mb-12 md:mb-16">
            001 / DOCTRINE
          </div>
        </Reveal>

        <RevealGroup stagger={0.18} className="space-y-2 md:space-y-3">
          <RevealItem
            as="p"
            className="font-extrabold text-[40px] sm:text-[56px] md:text-[88px] lg:text-[112px] leading-[1.02] tracking-[-0.025em] text-[var(--color-off-white)]"
          >
            This isn&apos;t a <span className="text-[var(--color-amber)]">chain gym.</span>
          </RevealItem>
          <RevealItem
            as="p"
            className="font-extrabold text-[40px] sm:text-[56px] md:text-[88px] lg:text-[112px] leading-[1.02] tracking-[-0.025em] text-[var(--color-off-white)]"
          >
            It&apos;s a <span className="text-[var(--color-amber)]">lifting club.</span>
          </RevealItem>
          <RevealItem
            as="p"
            className="font-extrabold text-[40px] sm:text-[56px] md:text-[88px] lg:text-[112px] leading-[1.02] tracking-[-0.025em] text-[var(--color-off-white)]"
          >
            Built for <span className="text-[var(--color-amber)]">the lift.</span>
          </RevealItem>
        </RevealGroup>

        <Reveal delay={0.6}>
          <p className="mt-12 md:mt-16 text-[var(--color-off-white)]/60 font-light text-[16px] md:text-[18px] tracking-wide max-w-[640px] leading-[1.55]">
            Veteran-owned. Family-operated. Open and staffed every hour, every day, at both stations.
          </p>
        </Reveal>
      </div>
    </section>
  );
}
