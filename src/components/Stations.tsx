...
function StationCard({ label, city, address, zipline, phone, mapsQuery, directionsUrl }: StationCardProps) {
  ...
  return (
    <div className="bg-[var(--color-canvas)] border border-[var(--color-steel)]/40 p-8 md:p-10 hover:border-[var(--color-amber)]/60 hover:-translate-y-1 transition-all duration-500">
      ...
      <a
        ...
        className="inline-flex items-center gap-3 px-6 py-3 border-[1.5px] border-[var(--color-amber)] text-[var(--color-amber)] font-bold text-[12px] tracking-[0.22em] hover:bg-[var(--color-amber)] hover:text-[var(--color-canvas)] transition-all duration-300"
      >
...
export function Stations() {
  return (
    <section id="stations" className="relative bg-[var(--color-carbon)] z-50 overflow-hidden py-32 md:py-48">
      <div className="relative w-full max-w-[1280px] mx-auto px-6 md:px-12">
        ...
        <RevealGroup stagger={0.2} className="grid md:grid-cols-2 gap-6 md:gap-8">
          <RevealItem y={40}>
            <StationCard
              label="STATION ONE"
              city="HESPERIA"
