...
export function ThePass() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Generate a pass number — replace with Supabase backend when keys are wired
    ...
    setSubmitted(true);
    // TODO: POST to /api/trial-pass when Supabase + Resend env vars are configured
  };

  return (
    <section id="the-pass" className="relative bg-[var(--color-canvas)] z-60 overflow-hidden min-h-screen flex items-center py-24 md:py-32">
          ...
          <>
            ...
            <div className="grid md:grid-cols-[1fr_320px] gap-12 lg:gap-20">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {["HESPERIA", "LA VERNE"].map((s) => (
                      <button
                        ...
                        className={`py-3 px-4 border-[1.5px] font-bold text-[12px] tracking-[0.22em] transition-all duration-200 ${
                          formData.station === s
                ...
                <button
                  type="submit"
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-8 py-4 bg-[var(--color-amber)] text-[var(--color-canvas)] font-bold text-[13px] tracking-[0.22em] hover:bg-[var(--color-amber-deep)] hover:shadow-[0_0_30px_-6px_rgba(244,160,36,0.5)] transition-all duration-250"
                >
              ...
              <div className="space-y-8 md:space-y-12 pt-2">
                <ProofBlock value="21K" label="INSTAGRAM COMMUNITY" />
                <ProofBlock value="24/7" label="STAFFED, EVERY HOUR" />
                <ProofBlock value="2018" label="VETERAN-OWNED" />
              </div>
            </div>
          </>
        ) : (
...
function ProofBlock({ value, label }: { value: string; label: string }) {
  return (
    <div>
